%yuying lai xuanzhou ma
clear
hold off
format long e
N = 4096; %No. of FFT samples
sampling_rate = 100.0e4; %unit Hz
...
Am =1 ;
mt = Am*sin(2*pi*4e3*tt);
%additional carrier
Ac1 = 0;
ct1 = Ac1*cos(2*pi*fc*tt);
st = mt.*ct + ct1;
...
axis ([-1.5e5 1.5e5 0 max(abs(Sf1))])
pause(1)
%Low pass filtering
f_cutoff = 30e3;
...
Hx=ylabel('|S hat(f)|');
set(Hx,'FontWeight','bold','Fontsize',16)
title('Spectrum S hat(f) after LPF');
axis ([-1.5e5 1.5e5 0 max(abs(Mf1))])
