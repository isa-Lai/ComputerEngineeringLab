    ...
    def connection_handler(self, client):
        ...
        while True:
            try:
                ...
                # search in database
                id = recvd_str.split()[0]
                cmd = recvd_str.split()[1]
                print("Received: " + cmd + " CMD from client " + id)
                send_byte = ""

                # search user
                user = self.search_database(id).copy()
                if user == None:
                    ...
                    print("User found.")
                key = user["Key"] # do not send key back
                del user["Key"]

                if cmd == "GG":
                    send_byte = str(user).encode('utf-8')
                else:
                    send_byte = str(self.search_average(cmd)).encode('utf-8')

                # encrypt message
                fernet = Fernet(key.encode('utf-8'))
                encrypted_message_bytes = fernet.encrypt(send_byte)
                
                # Send the received bytes back to the client. We are
                # sending back the raw data.
                connection.sendall(encrypted_message_bytes)
                print("Sent: ", send_byte)
