...
void generateSquare(std::vector<float> &t, std::vector<float> &x, float Fs, float interval ){
	float dt = 1/Fs;
	unsigned int totalSample = floor(interval*Fs);

	t.clear(); x.clear();
	t.resize(Fs,0.0); x.resize(Fs,0.0);

	//absolute random number
	srand((unsigned)time(NULL));
	unsigned int frequency = (rand()%16) +5; // frequency is random from 5Hz to 20Hz
	unsigned int magnitude = (rand()%5) +3;  // magnitude is random from 3 to 7

	float sw;
	for (unsigned int i = 0; i< totalSample; i++) {
		t[i] = i*dt;
		sw = std::sin(2*PI*frequency*t[i]);
		if(sw>0){
			x[i] = magnitude;
		}
		else{
...
int main()
{
	...
	//above graph will not be plotted
	// take home
	std::vector<float> square;

	generateSquare(t, square, Fs, interval);
	printRealVector(square);

	// declare a vector of complex values for DFT; no memory is allocated for it
	std::vector<std::complex<float>> Squaref;
	DFT(square, Squaref);

	std::vector<float> filter_square;              // filter out
	convolveFIR(filter_square, square, h);

	// compute DFT of the filtered data
	std::vector<std::complex<float>> Filter_squaref;
	DFT(filter_square, Filter_squaref);


	// prepare the data for GNU plot
	// please ignore the sv in this plot
	plotaddedSinesSpectrum(t, sv, square, filter_square, Squaref, Filter_squaref);
