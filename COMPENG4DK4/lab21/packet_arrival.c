...
//printf("packA %.5f %.5f\n",new_packet->arrive_time,new_packet->service_time);
  /* 
...
void
voice_arrival_event(Simulation_Run_Ptr simulation_run, void * ptr)
{
  Simulation_Run_Data_Ptr data;
  Packet_Ptr new_packet;
  ...
  new_packet = (Packet_Ptr) xmalloc(sizeof(Packet));
  new_packet->arrive_time = simulation_run_get_time(simulation_run);
  new_packet->service_time = (62.0*8.0+0.02*64000.0)/(double)(LINK_BIT_RATE);
  new_packet->status = WAITING;
  new_packet->type = 1;
