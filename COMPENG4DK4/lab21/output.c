...
output_progress_msg_to_screen(Simulation_Run_Ptr simulation_run)
{
  ...
  if((data->blip_counter >= BLIPRATE)
    ...
    printf("Successfully Xmtted Pkts  = %ld (Arrived Pkts = %ld) voice  = %ld (Arrived voice = %ld)\r", 
	   data->number_of_packets_processed, data->arrival_count, data->number_of_voice_processed, data->voice_arrival_count);
...
//voice
  ...
  xmtted_fraction = (double) data->number_of_voice_processed /
    data->voice_arrival_count;

  printf("Transmitted packet count  = %ld (Service Fraction = %.5f)\n",
	 data->number_of_voice_processed, xmtted_fraction);

  printf("Mean Delay (msec) = %.2f \n",
	 1e3*data->accumulated_voice_delay/data->number_of_voice_processed);
