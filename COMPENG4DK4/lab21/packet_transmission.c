...
end_packet_transmission_event(Simulation_Run_Ptr simulation_run, void * link)
{
  ...
  if (this_packet->type==0)
    ...
    {
    data->number_of_voice_processed++;
    data->accumulated_voice_delay += simulation_run_get_time(simulation_run) - 
      this_packet->arrive_time;
    }

//printf("out %d %.5f %.5f\n",this_packet->type,simulation_run_get_time(simulation_run),simulation_run_get_time(simulation_run)-this_packet->arrive_time );
...
{
  return exponential_generator((double) 0.04);
}
