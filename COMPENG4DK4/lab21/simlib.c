...
fifoqueue_put(Fifoqueue_Ptr queue_ptr, void * content_ptr)
{
 ...
 //printf("%d %d ", queue_ptr->size,((Packet_Ptr)content_ptr)->type);
#ifdef PRIORITY_MODE
  if (((Packet_Ptr)content_ptr)->type == 1)
    {
    if (queue_ptr->size == 0) {
      //printf(" put first\n");
    ...
    // put voice data in back
    else if (((Packet_Ptr)queue_ptr->back_ptr->content_ptr)->type == 1)
      {
      //printf(" put back\n");
      queue_ptr->back_ptr->next_ptr = queue_container_ptr;
      queue_ptr->back_ptr = queue_container_ptr;
      }
    // insert to after the last voice pack
    else if (((Packet_Ptr)queue_ptr->front_ptr->content_ptr)->type == 1)
      {
      int i =0;
      Queue_Container_Ptr tmp = queue_ptr->front_ptr;
      while(tmp->next_ptr != queue_ptr->back_ptr)
        {
        //printf("loop%d ", ((Packet_Ptr)tmp->next_ptr->content_ptr)->type);
        if (((Packet_Ptr)tmp->next_ptr->content_ptr)->type==0)
          break;
        tmp = tmp->next_ptr;
        i++;
        }
        // printf(" put %d\n",i);
      queue_container_ptr->next_ptr = tmp->next_ptr;
      tmp->next_ptr  = queue_container_ptr;
      }
    // insert at the first
    else  
      {
...
/*
  ...
  while(tmp!=NULL)
    {
    printf("%d ", ((Packet_Ptr)tmp->content_ptr)->type);
    tmp = tmp->next_ptr;
    }
  printf("\n");
  */
