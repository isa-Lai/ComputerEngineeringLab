...
call_arrival_event(Simulation_Run_Ptr simulation_run, void * ptr)
{
  ...
  if((free_channel = get_free_channel(simulation_run)) != NULL) {
    ...
    new_call->call_duration = exponential_generator((double) MEAN_WAIT_TIME);;
    ...
    schedule_call_meet_event(simulation_run,
...
/*
 * Function to schedule a call meet event. 
 */

long int
schedule_call_meet_event(Simulation_Run_Ptr simulation_run,
				   double event_time,
				   void * channel)
{
  Event new_event;

  new_event.description = "Call meet";
  new_event.function = call_meet_event;
...
/*
 * Call meet event function.
 */

void
call_meet_event(Simulation_Run_Ptr simulation_run, void * ptr)
{
  Call_Ptr this_call;
  Channel_Ptr channel;
  Simulation_Run_Data_Ptr sim_data;
  ...
  TRACE(printf("Customer and car meets.\n"););

  double giveup_time = exponential_generator((double) MEAN_GIVE_UP_TIME);

  if (now-this_call->arrive_time < giveup_time)
    {
    /*customer not give up */
    server_put(channel, (void*) this_call); //put back to channel
    double service_time = get_call_duration();
    this_call->call_duration+=service_time;
