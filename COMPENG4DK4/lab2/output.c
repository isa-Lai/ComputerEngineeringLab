...
output_results(Simulation_Run_Ptr simulation_run)
{
 ...
 //printf("Arrival rate = %.3f packets/second \n", (double) PACKET_ARRIVAL_RATE);
  ...
  printf("Mean Delay (msec) = %.2f %ld \n",
	 1e3*data->accumulated_delay/data->number_of_packets_processed, data->number_of_packets_processed);
  printf("Mean Delay1 (msec) = %.2f %ld \n",
	 1e3*data->accumulated_delays[0]/data->number_of_packets_processeds[0], data->number_of_packets_processeds[0]);
  printf("Mean Dela2 (msec) = %.2f %ld \n",
	 1e3*data->accumulated_delays[1]/data->number_of_packets_processeds[1],  data->number_of_packets_processeds[1]);
  printf("Mean Dela3 (msec) = %.2f %ld \n",
	 1e3*data->accumulated_delays[2]/data->number_of_packets_processeds[2],  data->number_of_packets_processeds[2]);
