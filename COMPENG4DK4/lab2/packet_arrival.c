...
schedule_packet_arrival_event(Simulation_Run_Ptr simulation_run,
			      double event_time, int bufferid)
{
  Event event;
...
void
packet_arrival_event(Simulation_Run_Ptr simulation_run, void * ptr, int bufferid)
{
  Simulation_Run_Data_Ptr data;
  Packet_Ptr new_packet;
  double arrival_rate[3] = {PACKET_ARRIVAL_RATE};
  ...
  new_packet->service_time = get_packet_transmission_time(bufferid);
  new_packet->status = WAITING;
  new_packet->orig_buffid = bufferid;

  //printf("A %d %.5f %.5f\n",bufferid, new_packet->arrive_time, new_packet->service_time);
  /* 
   * Start transmission if the data link is free. Otherwise put the packet into
   * the buffer.
   */

  if(server_state(data->link[bufferid]) == BUSY) {
    fifoqueue_put(data->buffer[bufferid], (void*) new_packet);
  } else {
    start_transmission_on_link(simulation_run, new_packet, data->link[bufferid], bufferid);
  }
  ...
  schedule_packet_arrival_event(simulation_run,
			...
			exponential_generator((double) 1/arrival_rate[bufferid]), bufferid);
}
